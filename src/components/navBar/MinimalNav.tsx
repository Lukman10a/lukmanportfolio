...
export function MinimalNav() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 py-4 lg:px-5 md:px-4 sm:px-3 sm:py-3 flex items-center justify-between">
        {/* Logo */}
        <motion.div
        ...
        >
          <Link href="/" className="text-2xl lg:text-xl md:text-lg sm:text-base font-bold tracking-tight">
            LUKMAN
        ...
        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center space-x-8 lg:space-x-6 sm:space-x-4">
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden relative z-50 text-black dark:text-white focus:outline-none p-3 sm:p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="fixed inset-0 bg-[#f8f4ec] dark:bg-[#121212] z-40 md:hidden flex flex-col justify-center items-center px-6"
            >
              <nav className="flex flex-col items-center space-y-8 lg:space-y-7 md:space-y-6 sm:space-y-5 text-center w-full max-w-xs sm:max-w-full">
                {navigationItems.map((item, index) => (
                  ...
                  >
                    <Link
                      href={item.href}
                      className={cn(
                        "block text-3xl sm:text-2xl font-medium py-2 transition-colors hover:text-brand",
                ...
                <motion.div 
                  ...
                  className="flex space-x-8 mt-10 sm:mt-8 sm:space-x-6"
                >
                  {socialLinks.map((link) => (
                    ...
                    >
                      ...
                      <SocialIcon name={link.icon} className="w-8 h-8 sm:w-7 sm:h-7" />
                    </motion.a>
                  ))}
                </motion.div>
              </nav>
...
function NavLink({ href, children, pathname }: { href: string; children: React.ReactNode; pathname?: string }) {
  ...
  return (
    <Link href={href} className="relative group">
      <span
        className={cn(
          "text-base lg:text-sm font-medium transition-colors",
